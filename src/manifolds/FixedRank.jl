...
# Types
# ---
struct FixedRank{m,n,k} <: Manifolds.Manifold{ℝ} end
...
function randomTVector(M::FixedRank{m,n,k}, x) where {m,n,k}
    F = svd(x)
...
# Tangent space
function project!(M::FixedRank{m,n,k}, ξ, x, q::AbstractMatrix) where {m,n,k}
    F = svd(x)
...
function project(M::FixedRank{m,n,k}, x, q::AbstractMatrix) where {m,n,k}
    ξ = zeros(size(x))
    return project!(M, ξ, x, q)
end

## TODO: improve this implementation and the handling of different representations of points/vectors
function project(M::FixedRank{m,n,k}, x, q::AbstractVector) where {m,n,k}
    X = reshape(x, (m, n))
    Q = reshape(q, (m, n))
...
function retract!(M::FixedRank{m,n,k}, y, x, ξ) where {m,n,k}
    F = svd(x + ξ)
    y = F.U[:, 1:k] * Diagonal(F.S[1:k]) * F.Vt[1:k, :]
    return y
end
function retract(M::FixedRank{m,n,k}, x, ξ) where {m,n,k}
    y = zeros(size(x))
...
function check_manifold_point(M::FixedRank{m,n,k}, x) where {m,n,k}
    X = reshape(x, (m, n))
...
function check_tangent_vector(M::FixedRank{m,n,k}, x, ξ) where {m,n,k}
    X = reshape(x, (m, n))
    Ξ = reshape(ξ, (m, n))
